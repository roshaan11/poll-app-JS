...
let currentPersonId=currentUser.userId;
...
let gettingAllInputs=()=>{
    ...
    inputs.forEach(input => {
        ...
        if (value) savingoptions.push(value);       
    }
);
...
class poll{
    constructor(pollTitle,pollDescription,pollOptions,timestamp,userId){
...
let pollCreation=()=>{
    ...
    const pollInfo=new poll(pollTitle.value,pollDescription.value,gettingAllInputs(),now.toDateString(),currentPersonId);
    savingData.push(pollInfo);

    localStorage.setItem("polls",JSON.stringify(savingData))
